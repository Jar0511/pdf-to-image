      ...
      <header className="relative z-10">
        <nav className="mx-auto flex w-full max-w-6xl items-center justify-between px-6 py-6">
          <div className="flex items-center gap-3">
            <div className="grid h-10 w-10 place-items-center rounded-xl bg-emerald-400/10 text-emerald-700 ring-1 ring-emerald-400/30 dark:text-emerald-200">
              P2I
            </div>
            <div>
              <p className="text-lg font-semibold tracking-tight">PDF To Image</p>
              <p className="text-xs text-slate-500 dark:text-slate-400">Client-side secure converter</p>
            </div>
          </div>
          <div className="hidden items-center gap-6 text-sm text-slate-600 md:flex dark:text-slate-300">
            <a className="transition hover:text-slate-900 dark:hover:text-white" href="#how">
              사용 방법
            </a>
            <a className="transition hover:text-slate-900 dark:hover:text-white" href="#security">
              보안 원칙
            </a>
            <a className="transition hover:text-slate-900 dark:hover:text-white" href="#faq">
              FAQ
            </a>
          </div>
          <Button
            type="button"
            variant="outline"
            className="border-slate-300 bg-transparent text-slate-700 hover:bg-slate-100 hover:text-slate-900 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-900 dark:hover:text-white"
      ...
      <main className="relative z-10">
        <section className="mx-auto flex w-full max-w-6xl flex-col gap-10 px-6 pb-16 pt-8 md:flex-row md:items-center md:pt-12">
          <div className="flex-1 space-y-6">
            <p className="inline-flex items-center gap-2 rounded-full border border-emerald-400/30 bg-emerald-400/10 px-4 py-1 text-xs text-emerald-700 dark:text-emerald-100">
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <label className="group relative flex cursor-pointer items-center justify-center rounded-2xl border border-dashed border-emerald-400/50 bg-emerald-400/10 px-6 py-4 text-sm font-medium text-emerald-700 transition hover:bg-emerald-400/20 dark:text-emerald-50">
                PDF 파일 선택
                <input className="absolute inset-0 hidden" type="file" accept="application/pdf" />
              </label>
              <button className="rounded-2xl border border-slate-300 px-6 py-4 text-sm font-medium text-slate-700 transition hover:border-slate-400 hover:text-slate-900 dark:border-slate-700 dark:text-slate-200 dark:hover:border-slate-500 dark:hover:text-white">
                변환 방식 알아보기
              </button>
            </div>
        ...
        <section id="how" className="mx-auto w-full max-w-6xl px-6 py-16">
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-3">
            {[
              ...
              <Card
                key={item.step}
                className="rounded-3xl bg-white/70 px-6 py-6 shadow-none dark:bg-slate-900/40"
              >
                <p className="text-xs text-emerald-600 dark:text-emerald-200">{item.step}</p>
        ...
        <section id="security" className="mx-auto w-full max-w-6xl px-6 py-16">
          <Card className="rounded-3xl bg-white/70 px-8 py-8 shadow-none dark:bg-slate-900/40">
            <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
              ...
              <div className="flex flex-col gap-3">
                <button className="rounded-2xl bg-slate-900 px-6 py-3 text-sm font-semibold text-white dark:bg-white dark:text-slate-900">
                  보안 안내서 보기
                </button>
                <button className="rounded-2xl border border-slate-300 px-6 py-3 text-sm text-slate-700 dark:border-slate-700 dark:text-slate-200">
                  개인정보 처리 없음
                </button>
              </div>
            </div>
      ...
      <footer className="relative z-10 border-t border-slate-200 dark:border-slate-800">
        <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-6 py-10 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-500">브라우저에서 끝나는 보안 변환 서비스</p>
          </div>
